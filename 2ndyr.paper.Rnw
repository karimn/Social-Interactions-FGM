...
We estimate
\begin{equation}
  y_{ijkt} = \alpha + \alpha_t + \beta_1 \bar{y}_{jkt} + \beta_2 \bar{m}_{jkt} + \beta_3 \bar{y}_{jkt} \times \bar{m}_{jkt} + \gamma x_{ijt} + \delta_j + \delta_{jt} + \theta_k + \theta_{kt} + \varepsilon_{ijkt}
\end{equation}
where $i$ refers to individual daughters; $j$ to a geographic region; $k$ to other groups such as wealth, religion, or education; and $t$ to the birth year.  $y \in \{0, 1\}$ is a binary choice variable reflecting the decision to circumcise $i$.  $m \in \{0, 1\}$ is another binary choice variable indicating whether the household opted for circumcision by a trained health provider or not. $x$ are $i$'s household characteristics.
