...
\title{The Social Effect of Medicalization\\on Female Genital Mutilation in Egypt}
\author{Karim Nagib}
...
The majority of research on the determinants of FGM has focused on individual characteristics \cite{Chesnokova2007, El-Gibaly2002}
...
require(car)
load("fgm.RData")
source("outreg-worked.R")

reg.filter <- function(regname)
{
  #if ((length(grep('governorate', regname)) > 0) |
  #    (length(grep('^birth\\.year\\.fac\\d{4}$', regname)) > 0))
  #  return(NULL)

  if (length(grep('^urban\\.ruralrural$', regname)) > 0)
    return('Rural')

  if (length(grep('^grpavg\\.circum$', regname)) > 0)
    return('$\\overline{fgm}_{j,k,t(n)}$')

  if (length(grep('^grpavg\\.has\\.or\\.intends\\.circum', regname)) > 0)
    return('$\\overline{fgm\\_or\\_intends}_{j,k,t(n)}$')

  if (length(grep('^grpavg\\.med$', regname)) > 0)
    return('$\\overline{med}_{j,k,t(n)}$')

  if (length(grep('^grpavg\\.circum_med$', regname)) > 0)
    return('$\\overline{fgm}_{j,k,t(n)} \\times \\overline{med}_{j,k,t(n)}$')

  if (length(grep('^educ\\.lvl', regname)) > 0)
    return(sub('^educ\\.lvl(\\w)(\\w+)$', 'Mother\'s education: \\U\\1\\E\\2', regname, perl = TRUE)) 

  if (length(grep('^marital\\.age', regname)) > 0)
    return('Mother\'s Marital Age')

  if (length(grep('^mother\\.circum\\.yes', regname)) > 0)
    return('Mother Circumcised')

  if (length(grep('^religionchristian', regname)) > 0)
    return('Religion: Christian')

  if (length(grep('^wealth\\.index', regname)) > 0)
    return(sub('^wealth\\.index(\\w)(\\w+)$', 'Wealth Index: \\U\\1\\E\\2', regname, perl = TRUE)) 

  return(NULL)
}

cohort.present <- function(coefs)
{
  return(c("Cohort Dummies", ifelse(length(grep("^birth.year.fac\\d{4}$", rownames(coefs))) > 0, "Yes", "")))
}

governorate.present <- function(coefs)
{
  return(c("Governorate Dummies", ifelse(length(grep("^governorate", rownames(coefs))) > 0, "Yes", "")))
}

cohort.governorate.present <- function(coefs)
{
  return(c("Cohort $\\times$ Governorate Dummies", ifelse(length(grep("^birth\\.year\\.fac\\d{4}:governorate", rownames(coefs))) > 0, "Yes", "")))
}

cohort.rural.present <- function(coefs)
{
  return(c("Cohort $\\times$ Rural Dummies", ifelse(length(grep("^birth\\.year\\.fac\\d{4}:urban\\.ruralrural", rownames(coefs))) > 0, "Yes", "")))
}

cohort.grpavg.circum.present <- function(coefs)
{
  return(c("Cohort $\\times \\overline{fgm}_{j,k,t(n)}$", ifelse(length(grep("^birth\\.year\\.fac\\d{4}:grpavg\\.circum", rownames(coefs))) > 0, "Yes", "")))
}

cohort.grpavg.med.present <- function(coefs)
{
  return(c("Cohort $\\times \\overline{med}_{j,k,t(n)}$", ifelse(length(grep("^birth\\.year\\.fac\\d{4}:grpavg\\.med", rownames(coefs))) > 0, "Yes", "")))
}

cohort.grpavg.circum_med.present <- function(coefs)
{
  return(c("Cohort $\\times \\overline{fgm}_{j,k,t(n)} \\times \\overline{med}_{j,k,t(n)}$", ifelse(length(grep("^birth\\.year\\.fac\\d{4}:grpavg\\.circum_med", rownames(coefs))) > 0, "Yes", "")))
}

cohort.grpavg.has.or.intends.present <- function(coefs)
{
  return(c("Cohort $\\times \\overline{fgm\\_or\\_intends}_{j,k,t(n)}$", ifelse(length(grep("^birth\\.year\\.fac\\d{4}:grpavg\\.has\\.or", rownames(coefs))) > 0, "Yes", "")))
}
@
