...
<<keep.source=TRUE, label=fgmdirect, echo=FALSE, results=tex>>=
  ...
  print.results.table(results, regs, cohort.range, educ.regs, NULL, cutpoints, symbols, other.fac = list(birth.order.fac.info), column.labels = c("OLS", "FE")) 
@
\begin{tablenotes}
  \item[1] Asterisks indicate significance levels: `$\cdot$' is 10 percent, `*' is 5 percent, `**' is 1 percent, and `***' is 0.1 percent.
  \item[2] In the OLS regression, fixed effects for governorates were also used.
\end{tablenotes}
\end{threeparttable}
\end{table}
