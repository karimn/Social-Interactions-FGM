...
\begin{itemize}
  ...
  \item $m^e_{ig}$ denotes the group's average behavior.  In Manski's \citeyearpar{Manski} notation it would be $E[\omega_{ig}|g]$. It can be should to be 
    \begin{equation}
...
\end{equation}
\citep{Blume2010} suggest relaxing the $y_g = \bar{x}_g$ assumption in order to make endogenous and contextual identification possible (to circumvent the reflection problem).  They suggest this could be done because of the $c \cdot \bar{x}_g/(1-J)$ term, that if it is not linearly dependent on a constant and $y_g$, breaks the reflection problem.  If it can be shown that
...
In our context we want to be able to identify the endogenous social effect of FGM.  Our model is
\begin{align}
...
\end{equation}
If we are able to show that some of the group averages of $x$ contribute to predicting $\bar{f}_{jt}$ beyond $(1, t, j)$, we can use them to instrument $\bar{f}_{jt}$, and hence be able to identify the endogenous effect of FGM.  The below results show that the group averages for urban residence, the mother having received primary education and secondary education, and the mother's having FGM status are significant.  Therefore, we use them as instruments for $\bar{f}_{jt}$.
...
\subsubsection{FGM Endogenous Effects}

Below are the results for the IV regression.  They show a positive and significant endogenous effect.

<<echo=TRUE>>=
r1.instr.2$regress.formula
r1.instr.2$summary()
@

Additionally, we use the different daughters in the same household to run a panel data regression to eliminate unobservable household effects.  The results again show a positive and significant endogenous effect.

<<echo=TRUE>>=
r2.instr.2$regress.formula
r2.instr.2$summary()
@

\subsubsection{Medicalization Effects}
...
\end{align}
where $m$ is a binary variable indicating the household's decision to opt for medicalized FGM as opposed to traditional.  \emph{If} we can make the assumption that $L^m = 0$, we can define
    \[m^e_{jht} = \bar{m}_{(-i)jt}.\]
(We exclude the $i^{th}$ observation to avoid possible endogeneity).

Below are the results of the same IV regression as above but including medicalization.  As shown, there is no significant effect to either medicalization or FGM.

<<echo=TRUE>>=
r3.instr.2$regress.formula
r3.instr.2$summary()
@

However, if we run a panel data regression, as shown below, we see they both become significant, and most interestingly, medicalization has a negative effect on the FGM decision.

<<echo=TRUE>>=
r4.instr.2$regress.formula
r4.instr.2$summary()
@
