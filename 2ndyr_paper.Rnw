...
cat(sprintf("All & 100\\%% & %.2f\\%% & %.2f\\%% \\\\ \n", circum.frac * 100, med.frac * 100))
...
cat(sprintf("\\\\\nBy Religion (Christian) & %.2f\\%% & %.2f\\%% & %.2f\\%% \\\\*\n", 
...
fgm.christ <- fgm[fgm$religion == "muslim", ]

cat(sprintf("\\\\\nBy Religion (Muslim) & %.2f\\%% & %.2f\\%% & %.2f\\%% \\\\*\n", 
            nrow(fgm.christ)/nrow(fgm)*100,
            nrow(fgm.circum[fgm.circum$religion == "muslim",])/nrow(fgm.christ)*100,
            nrow(fgm.med[fgm.med$religion == "muslim",])/nrow(fgm.christ)*100))
...
Table \ref{tab:sumstat} gives descriptive statistics for the considered sample, stratified by wealth level, urban/rural residence, and religion.  What is noticeable is a decrease in the rate of overall circumcision with some fluctuation in the rate of medicalization (this can also be seen in figure \ref{fig:fgmprop}).  We also observe greater tendency for FGM among the poorer\footnote{EDHS 2008 data originally divides the wealth index of households into five levels, but in the interest of simplifying our analysis they are condensed into two levels: poor and rich.}, rural and Muslim households.  Also is noticeable a greater tendency for medicalization for poorer, urban, and Muslim households.
...
cat(sprintf("\\begin{longtable}{*{%i}{l}}\n", 1 + length(govern.names)))
cat("\\caption{Governorate Summary Statistics\\label{governsumstat}} \\\\\n")
cat("\\hline\n\\hline\n")
cat("& \\begin{sideways}Circumcized\\end{sideways}")
cat("& \\begin{sideways}Medicalization\\end{sideways}")
cat("& \\begin{sideways}Wealth Index (Rich)\\end{sideways}")
cat("& \\begin{sideways}Rural Residence\\end{sideways}")
cat("& \\begin{sideways}Religion (Christian)\\end{sideways}")
cat("& \\begin{sideways}Mother Circumcized\\end{sideways}")
cat("& \\begin{sideways}Mother's Marital Age\\end{sideways}")
cat("& \\begin{sideways}Age of Circumcision\\end{sideways}")
cat("\\\\\n")
cat("\\hline\n")
cat("\\endfirsthead\n")
cat(sprintf("\\multicolumn{%i}{c}{{\\tablename} \\thetable{} -- Continued} \\\\\n", length(govern.names) + 1))
cat("\\hline\n\\hline\n")
cat("& \\begin{sideways}Circumcized\\end{sideways}")
cat("& \\begin{sideways}Medicalization\\end{sideways}")
cat("& \\begin{sideways}Wealth Index (Rich)\\end{sideways}")
...
cat(sprintf("All & %.2f\\%% & %.2f\\%% & %.2f\\%% & %.2f\\%% & %.2f\\%% & %.2f\\%% & %.2f (%.2f) & %.2f (%.2f) \\\\\n",
...
for (n in govern.names)
{
  ...
  cat(sprintf("%s & %.2f\\%% & %.2f\\%% & %.2f\\%% & %.2f\\%% & %.2f\\%% & %.2f\\%% & %.2f (%.2f) & %.2f (%.2f) \\\\\n",
              n,
...
fgm.results.outreg(results.circum, var.dict, title = "Circumcision Social Effects Estimate Results", label = "tab:circum.reg", exclude.foot = TRUE)
@
