...
\begin{tablenotes}
  ...
  \item[2] Other regressors not shown: governorate fixed effects, cohort (year of birth) fixed effects, order of birth fixed effects, `modernization' proxy variables, and peer group means of exogenous household characteristics (exogenous social effects).
\end{tablenotes}
\end{threeparttable}
\end{table}
\end{landscape}
