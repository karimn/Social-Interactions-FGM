...
names(fgm.data.all.08) <- 
  c('phase', 'cluster', 'hh', 'respond.linenum', 'area.unit', 'wt', 'birth.year', 'age', 'age.grp', 'domain', 
    ...
    'circum', 'circum.age', 'circum.bywhom', 'circum.daughters.num', 'circum.anydaughter.not', 'circum.intends', 'circum.byreligion', 'circum.continue', 'governate', 'prev.governate', 
    'husband.related', 'husband.relation', 'circum.daughternot.num')

fgm.data.all.08 <- transform(fgm.data.all.08, circum = factor(circum, levels = c(0, 1), labels = c("no", "yes")),
                                        dhs.year = 2008)
...
fgm.data.all.08 <- within(fgm.data.all.08, {
  ...
  partner.educlvl <- factor(partner.educlvl, levels = c(0:5, 8, 9), labels = c("no education", "incomplete primary", "complete primary", "incomplete secondary", "complete secondary", "higher", "don't know", "missing"), exclude = NA)
