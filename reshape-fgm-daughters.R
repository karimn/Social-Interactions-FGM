...
names(fgm.data)[1:14] <- c('phase', 'cluster', 'hh', 'respond', 'area.unit', 'wt', 'domain', 'region', 'urban.rural', 'years.lived.res', 'religion', 'hh.head.sex', 'wealth.index', 'intends.circum')
...
fgm.data <- merge(fgm.data, pr, by.x = c('cluster', 'hh', 'respond'), by.y = c('hv001', 'hv002', 'hv003'))
rm(pr)

names(fgm.data)[names(fgm.data) == 'hc61'] <- 'mother.educlvl'
names(fgm.data)[names(fgm.data) == 'hc62'] <- 'mother.educyr'

fgm.data <- transform(fgm.data, mother.edulvl = factor(mother.educlvl, levels = c(0:3, 9), labels = c("no education", "primary", "secondary", "higher"), exclude = 9))

fgm.data <- subset(fgm.data, select = c(cluster:hh.id, birth.year, weight, height, sdno, mother.educlvl, mother.educyr)) 
...
fgm.data$religion <- factor(fgm.data.08$religion, labels = c("Muslim", "Christian", "Missing"))
