...
fgm.data.all.08 <- transform(fgm.data.all.08, med.circum = ifelse((circum == 1) & (circum.bywhom %in% c("doctor", "trained nurse/midwife")), 1, 0),
...
#fgm.data.all.08 <- transform(fgm.data.all.08, circum.timeperiod = factor(ifelse(circum.year > "2007", 0,
#                                                                     ifelse(circum.year >= "1997", 1, 
#                                                                       ifelse(circum.year < "1997", 2, NA))), labels = c("After 2007", "1997-2007", "Before 1997")),

 #fgm.data.all.08 <- transform(fgm.data.all.08, religion = factor(religion, labels = c("Muslim", "Christian", "Missing")))

gps.08 <- read.dbf("~/Data/EDHS/2008/EGGE5BFL.DBF")
#gps.08 <- transform(gps.08, unique.cluster = factor(paste(DHSYEAR, DHSCLUST, sep = '-')))

#cluster.coordinates(fgm.data.all.08) <- gps
#rm(gps)
